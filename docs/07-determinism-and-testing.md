...
Runners close the loop: they turn `Action`s into real work and completions back into `ProtocolEvent`s. Actions split into inline (timer registry mutations, continuations), delegated (CPU work — signature verification, QC assembly, transaction execution — dispatched to pools via the `Dispatch` trait), network, and persistence. Adding a variant forces updating its routing classification, so the sans-io boundary is maintained mechanically.

Transaction execution itself is batch-shaped (`Executor` in `crates/engine`): the unit is a *tick*, one batch per committed block carrying work — that block's own transactions that can run at once, plus every cross-shard leg whose provisions completed at the same commit. The whole batch goes to the kernel's deterministic-parallel executor at once, and it returns one receipt per transaction in input order. Those receipts are schedule-invariant — group scheduling (serial, parallel, adversarial timing) cannot change any receipt byte, so the scheduling mode is a performance knob and never a consensus input. Guests run on the pinned wasmtime engine natively and on the reference interpreter on wasm32, with byte-identical receipts and fuel across both runtimes held by the VM repository's differential suite — determinism at the execution boundary is verified by two independent implementations, not assumed.

Each tick executes against the previous tick's output laid over the settled base, so a transaction reads what the block before it wrote without waiting for that block to settle. **A tick's output is a pure function of the committed chain prefix** — not of when this node happened to run it. Execution trails consensus by an unbounded margin, so two honest validators on one chain are routinely at different tick positions; if local timing reached a receipt they would derive different writes, and the slower one would silently stop contributing to every tick it ran. Four things keep that from happening, and each is a place the property could have been lost:

- Every tick reads the base **no later than its own height**. A validator whose execution lags has already persisted settlements belonging to ticks it has not run, and folding those into an earlier tick's baseline is exactly the leak.
- The base and the folds over it are **disjoint**, never overlapping. A receipt says what an exclusive write left and what a commutative access moved, and only the first of those survives being applied twice — so each fold records the height its tick's settlement reaches the base at, and a read takes the fold or the base according to where its anchor falls, never both. Which side of that line a validator's persistence happens to sit on therefore cannot change the answer.
- A cell an unresolved cross-shard leg has written is **claimed**: nothing may read it until that tick's fate commits, because the value there is one an abort could retract. Transactions declaring a claimed cell wait, so no tick has a member whose read a later promotion could move.
- A fold is retained until **both** persistence and execution have passed the height its tick settled at. Dropping it on persistence alone would let eviction outrun a queued tick, which would then find neither the fold nor a base old enough to hold it.

The overlay is local and stays that way: it never feeds state-root computation, which remains settlement-derived. When local execution does disagree with its committee — the voted receipt root against the one certified — the node names the first divergent tick and stops. Deterministic re-execution reproduces a deterministic disagreement, so there is nothing to retry.
...
| `WeightedTimestamp` | Quorum-attested clamped average, monotone per chain | Everything consensus-critical: committee resolution, validity windows, finalization deadlines, DA retention, reshape boundaries ([01-consensus-layers.md](01-consensus-layers.md) §1.3) |
