...
Each tick executes against the previous tick's output laid over the settled base, so a transaction reads what the block before it wrote without waiting for that block to settle. **A tick's output is a pure function of the committed chain prefix** — not of when this node happened to run it. Execution trails consensus by an unbounded margin, so two honest validators on one chain are routinely at different tick positions; if local timing reached a receipt they would derive different writes, and the slower one would silently stop contributing to every wave it held. Three things keep that from happening, and each is a place the property could have been lost:

- Every tick reads the base **as of its own height**, never as of whatever this node has persisted. A validator whose execution lags has already persisted settlements belonging to ticks it has not run.
