...
Liveness is protected by construction rather than by retry. Local transactions cannot deadlock because nothing holds them apart: everything a block commits executes as one batch, sequenced by the executor over a single overlay. And every wave carries a deadline derived from BFT-attested time, after which it deterministically all-aborts — which is also what breaks a true cross-shard cycle, since every replica derives the same deadline from attested content alone. Every transaction terminates; no lock is held forever; no two shards ever disagree on the terminal outcome.
