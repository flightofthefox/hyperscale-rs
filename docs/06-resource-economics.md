...
That reservation is an accounting entry over the payer shard's own committed chain, not a hold on the vault. Nothing is moved when it engages, and it resolves exactly once when the transaction finalizes: the attested actual burns on success and the class floor on abort, both written **inside the settling receipt** rather than beside it, so a replica that reconstructs state by replaying receipts reconstructs the burn with it. The remainder is released by the entry ceasing to be derivable (INV-VM-HOST-2). Every fee burns; none is paid to anyone.
