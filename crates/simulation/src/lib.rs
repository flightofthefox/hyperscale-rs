...
///
/// Under the `ci` feature the sims use the production 5-minute epoch and
/// sign with real BLS, per [`CryptoScheme::default`]. The
...
/// quickly, and the validators sign with the mock scheme. The
/// recovery-timeout ratio is no longer production-faithful, so the
/// epoch-counted reshape budgets and TTLs can behave differently than they
/// do in production — run with `--features ci` before trusting those.
#[cfg(not(feature = "ci"))]
