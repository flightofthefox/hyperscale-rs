...
fn equivocation_round_trip_round1() {
    ...
    assert!(verify_vote_equivocation(&BlsVerifier, &ev, &network, &cm.members).is_ok());
}
...
fn qc1_over(
    ...
    let refs: Vec<&PcVote1> = votes.iter().collect();
    build_qc1(&BlsVerifier, &refs, &cm.members)
}
...
// ─── QC3 endpoint forgery ──────────────────────────────────────────────────────
//
// `verify_qc3` must reject forged QC3s with prover-chosen `x_pp` / `x_pe`
// endpoints, even when the embedded round-2 QCs and the aggregate signature are
// real. Harvest a legitimate QC3 by driving `PcSim` to consensus, then swap in a
// bogus endpoint — verification fails because the endpoint-claim against
